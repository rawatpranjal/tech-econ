...
### ALWAYS snapshot WIP to `/tmp` before stash cycles and verify after pop
- **Why:** 2026-05-25 session lost 5 prior-session CHANGELOG entries during a stash → reset → pop chain across PRs #52, #54, #55. `git stash pop` ran cleanly with no conflict reported, but the CHANGELOG changes were silently dropped. Recovered only because the entries were also backed up at `/tmp/changelog_wip_full.md`. The 10 other modified files survived; CHANGELOG was the casualty.
- **How to apply:** If you must carry working-tree WIP across multiple stash → branch → pop cycles, `cp -r` the full set of modified files to `/tmp/<session>-wip/` before the first `git stash push`. After each `git stash pop`, `diff -r /tmp/<session>-wip/ .` for the files you expected back. Stash is not a reliable transport for multi-cycle WIP carry-over.
