...
| **§4 A/B testing harness** (A1-A8) | TODO | **DONE** | Client-side bucketing in PR #18 (`data/experiments.json` + `static/js/experiments.js` + inlined config in `baseof.html` + deterministic `te_uid + experiment_id` bucketing, 30 vitest tests). PR #18 needed a `safeJS` fix during merge-check — a Hugo auto-escape bug had silently double-encoded the inlined config (caught by parsing rendered HTML; see `RULES.md` "ALWAYS verify rendered HTML" for the codified protocol). |
| **§4 server-side** (tracker logging + D1 schema + `analyze_experiments.py`) | TODO | **DONE** | PR #42 (tracker.js attaches `event.exp`) → PR #46 (worker `events.experiments` column + INSERT path, **deployed**) → PR #47 (`scripts/analyze_experiments.py`: per-variant CTR + Wilson 95% CI + two-prop z-test + verdict heuristic, 31 tests). Smoke-tested end-to-end against live D1: probe event → `json_extract(experiments, '$._example_disabled')` returned `[{"variant":"control","n":1}]`. |
| **First real experiment running** | TODO | **DONE** | PR #48: `harness_aa_v1` A/A test (`control_a` / `control_b` 50/50). Both variants do the same thing — pure pipeline-validation. After a few days, run `python3 scripts/analyze_experiments.py --experiment harness_aa_v1`; expect ~50/50 split. 4 new vitest cases (10k synthetic-uid distribution + determinism + config integrity). |
...
| **`lib/d1_client.py`** (HTTP wrapper for analytics worker) | n/a (added during impl) | **DONE** | PR #13. Read-only typed endpoints; injectable `FakeFetcher` for tests. |
| **`lib/freshness.py`** (extract freshness from rank_all_content.py) | n/a (added during impl) | **DONE** | PR #17 (extracted) + PR #43 (`rank_all_content.py` migrated to wrapper) + PR #44 (constants moved to `data/recsys_config.json`). Ready for per-type half-lives via `Mapping[type → half_life_days]`. |
| **`scripts/scoreboard_status.py`** (recsys scoreboard one-screen view) | n/a (added during impl) | **DONE** | PR #41. Reads `metrics.csv` + `replays.csv`; prints latest row, deltas, holdout-mix WARNING, and a RED/YELLOW/GREEN decision-readiness guard for default flips. |

**Score:** 16 done · 1 partial (Ra2 default flip) · 4 todo (R4 multi-channel retrieval, R5 Item2vec, Re3 position-bias, Re5 bandit) · 1 N/A (R1) · 5 deferred.

**Next decision point:** §4 A/B harness is now end-to-end alive with the `harness_aa_v1` A/A test running. After a few days of traffic, run the analyzer and confirm ~50/50; once validated, the audit's progression points to either:
- **R4 multi-channel retrieval + R5 Item2vec** (architectural shift; longer-term)
- **Re3 position-bias correction** (pairs with Ra3 click-rank logging — already in place; needs more data first)
- **Ra2 default flip to `knn-bge`** (premature: scoreboard at 1 row, decision guard says RED — needs ≥2-3 rows at the same `holdout_days`)

The actual highest-leverage thing is to **let the A/A run for a few days**, then **launch the first real treatment-vs-control experiment** (e.g., MMR λ tuning at the search worker, or personalize.js LAMBDA tuning) using the harness that's now live.
