...
## 2026-05-04
- **§4 server-side scaffolding step 2 — `analytics-worker/index.js`**: adds `events.experiments` column (JSON map of `{experiment_id: variant_id, ...}`) plus a sparse index on `experiments IS NOT NULL`. INSERT path serializes `event.exp` from the tracker payload (PR #42); migration mirrored in `ensureSchema` (per-isolate self-heal) and `handleRunSchema` (cross-isolate idempotent ALTER) per RULES.md F15. **Requires deploy** (`npx wrangler deploy` from `analytics-worker/`) followed by `GET /run-schema?key=$ADMIN_KEY` to apply the migration to the live D1. `schema.sql` also updated for fresh installs.
